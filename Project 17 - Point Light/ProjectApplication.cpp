...
void ProjectApplication::DrawCubes()
{
	for (int i = 0; i < CUBE_COUNT; ++i)
	{
		...
		cubeShader.SetUniformVector3("pointLight.position", lightModelMatrix.GetPosition());

		cubeShader.SetUniformVector3("pointLight.ambient", glm::vec3(0.2f, 0.2f, 0.2f));
		cubeShader.SetUniformVector3("pointLight.diffuse", glm::vec3(0.5f, 0.5f, 0.5f));
		cubeShader.SetUniformVector3("pointLight.specular", glm::vec3(0.5f, 0.5f, 0.5f));

		cubeShader.SetUniformFloat("pointLight.constant", 1.0f);
		cubeShader.SetUniformFloat("pointLight.linear", 0.09f);
		cubeShader.SetUniformFloat("pointLight.quadratic", 0.032f);
